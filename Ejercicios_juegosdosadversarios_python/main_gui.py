...
class TicTacToeGUI:
    ...
    def __init__(self, ventana):
        ...
        if messagebox.askyesno(
            "¿Quién comienza?", "¿Desea que la IA (X) empiece la partida?"
        ):
    ...
    def ActualizarGUI_tablero(self):
        for r in range(3):
            for c in range(3):
                ...
                self.botones[(r, c)].config(
                    text=self.visual[valor], fg=self.colores_jugador[valor]
                )
